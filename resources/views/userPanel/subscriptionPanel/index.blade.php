<x-app-layout>
    ...
    <div class="mt-3">
      <a class="text-white px-4 py-2 mb-3 w-auto h-10 bg-green-600 rounded-full hover:bg-green-700 active:shadow-lg mouse shadow transition ease-in duration-200 focus:outline-none"
       href="{{route('sub.newOrder',$sub->id)}}">
            ...
            <table class="w-full">
              ...
              <tbody class="bg-white">
                ...
                <tr class="text-gray-700">
                    ...
                    <td class="px-4 py-3 text-sm border-t"><a class="text-white px-4 py-3 w-auto h-10 bg-black rounded-full hover:bg-gray-700 active:shadow-lg mouse shadow transition ease-in duration-200 focus:outline-none" href="{{ route("sub.show",$order->id) }}">
                      {{ __('Show') }}
                    </a></td>

                        @if ($sub->package->number_of_works != $sub->done)
              <td class="px-4 py-3 text-sm border-t"><a class="text-white px-4 py-3 w-auto h-10 bg-blue-600 rounded-full hover:bg-blue-700 active:shadow-lg mouse shadow transition ease-in duration-200 focus:outline-none" href="{{ route("sub.edit",$order->id) }}">
                      {{ __('Edit') }}
                    </a></td>
                    @else
                    <td></td>
                    ...
                    @if($order->work_status == 'Accepted' && $order->work_submitted == 'submitted') 
                    @if($order->evaluation)
                ...
                <dialog id="rate{{$order->id}}" class="bg-transparent z-0 relative w-screen h-screen">
                  <form action="{{route('sub.reviews',$order->id)}}" enctype="multipart/form-data" method="post">
                    @csrf
